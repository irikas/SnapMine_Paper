...
This is what the output results look like (long format). The long format contains up to (n+1) columns:
-sampleID (Snaptron sample ID)
...
result_df_wide_subset <- result_df_wide %>% 
  mutate(maxPSI = apply(result_df_wide[,-1], MARGIN = 1, 
...
ggplot(result_df_wide_subset, aes(x = maxPSI, fill=sampleID))+
  geom_histogram(binwidth = 1, color = "white", linewidth = 0.1)+
  theme_classic()+
...
# Subset to avgPSI > 15 for 25% or more of targets
result_df_wide_TDPpos <- result_df_wide_subset %>% dplyr::select(-maxPSI) %>% 
  mutate(n15 = apply(result_df_wide_subset[,-1], MARGIN = 1, 
...
result_df_long_TDPpos <- result_df_long_subset %>% filter(sampleID %in% result_df_wide_TDPpos$sampleID) %>%
...
ggplot(result_df_long_TDPpos, aes(x = sampleID, y=avgPSI))+
  ...
  theme_classic()+
  ggtitle(label = "Samples that are putatively lacking nuclear TDP-43", 
          subtitle = "Contains CE in 10% of tested samples")+
  theme(legend.position = "none")
