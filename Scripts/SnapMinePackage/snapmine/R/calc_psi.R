...
calculatePSI <- function(novel_junc_coord, canon_junc_coord, snaptron_df, totalCountMin = 15, filter_noCanon = T){
  if(!is.na(novel_junc_coord)){
    ...
    counts_df <- canonCounts %>% full_join(novelCounts, by = "sampleID", suffix = c("_canon", "_novel")) %>%
      mutate(junctionCount_canon = ifelse(is.na(junctionCount_canon), 0, junctionCount_canon),
    ...
    # Calculate PSI
    counts_df <- counts_df %>%
      mutate(psi = junctionCount_novel/(totalCounts),
             psi = round(psi * 100,2)) %>%
