...
for(i in 1:nrow(info)){
  dftemp <- calcPSI(chr = info$chromosome[i], incA = info$inc1_A[i], incB = info$inc1_B[i],
                incC = info$inc2_C[i], incD = info$inc2_D[i], 
                excA = info$exc_A[i], excD = info$exc_D[i], genome = "srav1m")
...
sampleMeta <- samplesInfo(genome = "srav1m", sampleIDs = df$sampleID)
df <- merge(df,sampleMeta,by.x = "sampleID", by.y = "rail_id")

write.csv(df, "AllCEPSI_Mouse_Meta.csv")
