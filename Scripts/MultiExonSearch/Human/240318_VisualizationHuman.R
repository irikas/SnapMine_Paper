...
df <- df %>% pivot_longer(cols=!contains("sampleID"),names_to = "Junction",values_to = "PSI") %>% 
  filter(Junction %in% c("ACTL6B","AGRN_long","ATG4B_long","G3BP1","MYO18A","PFKP","RANBP1","STMN2","UNC13A","UNC13B"))
...
ggsave(plot = p, "240318_srav3h.png",device = "png",width = 30, height = 20, units = "cm",limitsize = FALSE)
